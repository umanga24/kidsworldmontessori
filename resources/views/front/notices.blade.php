            ...
            <div class="col-md-6 col-lg-4 mt-2-2">
                <article class="card card-style-04 h-100 rounded-0">
                    <div class="blog-img position-relative overflow-hidden">
                        <img src="{{asset('images/thumbnail/'.$notice->image)}}"
                            alt="notice">
                       
                    </div>
                    <div class="card-body p-1-9">
                        <span class="text-primary d-block mb-2 font-weight-600">{{ $notice->created_at->format('F d, Y') }}</span>
                        <h3 class="h4 mb-0"><a href="">{{$notice->title}}</a>
                        </h3>
                    </div>
                    <div class="d-flex fw-bold border-top px-1-9 py-3 border-color-light-black justify-content-between">
                        <a href="{{route('noticeInner',$notice->slug)}}">Read more</a>
                        <a href="{{route('noticeInner',$notice->slug)}}"><i class="ti-arrow-right"></i></a>
                    </div>
                </article>
            </div>
            @endforeach
